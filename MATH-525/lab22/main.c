...
int main(int argc , char* argv [])
{
    ...
    //Stop until all procs reaches here
    ...
    //Divide the norm from loacal vetor
    ...
    //Save first and last values of "Big vetor" AFTER computing the 2-norm in local norm
    if ( my_rank != 0)
    {
        if (my_rank == comm_sz-1)
        {
    ...
    if ( my_rank ==0)
    {
        ...
        printf (" Results with MPI_Barrier\n");
